...
// THIS BELONGS IN THE "NAVIGATETORESULTS FUNCTION!!!!!"
// once the loader is complete, look for flagged favorites and push 
// them into our cards array and then set a cookie with each cards ID 
// to be used on the results page
var previewCard = $('.preview-card'),
    cards       = [];
		
$.each(previewCard, function(i) {
	if ($(previewCard[i]).hasClass('favorite')) {
		cards.push($(previewCard[i]).find('.id').text());
	}
});
setCookie('favorite', cards);
